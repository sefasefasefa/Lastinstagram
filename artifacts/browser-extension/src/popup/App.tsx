...
export function App() {
  ...
  return (
    <div className="w-[280px] bg-background text-foreground font-sans p-5 flex flex-col gap-5 border border-border selection:bg-primary/30 relative overflow-hidden">
      ...
      <div className="absolute -top-10 -right-10 w-40 h-40 bg-primary/10 rounded-full blur-[64px] pointer-events-none" />

      <div className="flex items-center gap-3 relative z-10">
        <div className="w-10 h-10 rounded-xl bg-card border border-white/5 flex items-center justify-center shadow-lg shadow-black/50 flex-shrink-0">
          <Instagram className="w-5 h-5 text-primary drop-shadow-[0_0_8px_rgba(225,48,108,0.5)]" />
        </div>
        <div className="flex flex-col min-w-0">
          <h1 className="text-sm font-semibold tracking-tight truncate">Nexus Control</h1>
          <p className="text-[10px] font-mono text-muted-foreground uppercase tracking-widest truncate mt-0.5">Operator Node</p>
        </div>
      </div>

      <div className="rounded-xl bg-card/40 backdrop-blur-xl border border-white/5 p-4 flex flex-col gap-4 shadow-inner relative z-10">
        ...
        {igStatus === "missing" && (
          <button
            ...
            className="w-full flex items-center justify-between px-3.5 py-2.5 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 text-[11px] font-semibold uppercase tracking-wider transition-all text-white group active:scale-[0.98]"
          >
      ...
      <button
        onClick={openPanel}
        className="relative w-full flex items-center justify-center gap-2 py-3 rounded-lg bg-primary hover:bg-primary/90 text-primary-foreground text-xs font-semibold tracking-wide transition-all shadow-[0_0_15px_rgba(225,48,108,0.25)] hover:shadow-[0_0_25px_rgba(225,48,108,0.4)] active:scale-[0.98] group overflow-hidden z-10"
      >
        {/* Button inner glare */}

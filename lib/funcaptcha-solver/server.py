...
class _NodeJsRunner:
    ...
    def call(self, fn, *args):
        import json as _j
        args_s = ", ".join(_j.dumps(a) for a in args)
        script = self.source_code + "\nconst __r=" + fn + "(" + args_s + ");process.stdout.write(JSON.stringify(__r));"
        with _tempfile.NamedTemporaryFile(mode="w", suffix=".js", delete=False, dir="/tmp") as f:
            f.write(script)
            tf = f.name
        try:
            p = _subprocess.run(["node", tf], capture_output=True, text=True, timeout=30)
            if p.returncode != 0:
                raise RuntimeError("node error: " + p.stderr[:300])
            return _j.loads(p.stdout)
        finally:
            try: _os_path.unlink(tf)
            except: pass

_NODE_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..", ".config", "npm", "node_global", "node_modules")

def _run_in_jsdom(dapib_code, answers, user_agent):
    import json as _j, tempfile as _tf, subprocess as _sp, os as _o
    answers_json = _j.dumps(answers)
    # strip index keys for grid vs standard
    answers_for_js = answers_json.replace('"index"', 'index')
    script = """
const {{ JSDOM, ResourceLoader }} = require("jsdom");
const {{ Script }} = require("vm");
const rl = new ResourceLoader({{ userAgent: {ua_json} }});
const dom = new JSDOM("", {{ runScripts: "dangerously", resources: rl, pretendToBeVisual: true, storageQuota: 10000000 }});
const vm = dom.getInternalVMContext();
new Script(`response=null;window.parent.ae={{"answer":{answers_for_js}}};window.parent.ae[("dapibRecei"+"ve")]=function(d){{response=JSON.stringify(d);}};`).runInContext(vm);
new Script({dapib_json}).runInContext(vm);
const r = new Script("response").runInContext(vm);
process.stdout.write(r || "null");
""".format(ua_json=_j.dumps(user_agent), answers_for_js=answers_for_js, dapib_json=_j.dumps(dapib_code))
    with _tf.NamedTemporaryFile(mode="w", suffix=".js", delete=False, dir="/tmp") as f:
        f.write(script)
        tf = f.name
    try:
        env = dict(_o.environ)
        env["NODE_PATH"] = _NODE_PATH
        p = _sp.run(["node", tf], capture_output=True, text=True, timeout=30, env=env)
        if p.returncode != 0:
            raise RuntimeError("jsdom error: " + p.stderr[:300])
        return _j.loads(p.stdout)
    finally:
        try: _o.unlink(tf)
        except: pass

_arkose_js_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "arkose.js")
with open(_arkose_js_path) as file:
    gctx = _NodeJsRunner(file.read())
...
class Arkose:
    ...
    def t_guess(self, session_token:str, guesses:list, dapibCode:str) -> str:
        ...
        result = _run_in_jsdom(
            dapibCode,
            answers,
            f"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{self.chrome_version}.0.0.0 Safari/537.36"
        )
    ...
    @staticmethod
    def generate_key(ctx, s_value:str, useragent:str) -> list:
...
class _solver_stats:
    ...
    def title():
        ...
        while True:
            ...
            pass  # Windows title update (skipped on Linux)
            time.sleep(1)

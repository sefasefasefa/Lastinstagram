...
- Added an interactive Instagram **checkpoint** (security-verification) resolution flow (2026-07-15): `/auth/checkpoint/options` → `/auth/checkpoint/select-method` → `/auth/checkpoint/verify`, backed by `fetchChallengeContext`/`selectChallengeMethod`/`submitChallengeCode` in `lib/instagram-client/src/direct-login.ts`. This is separate from the existing 2FA flow (`/auth/verify-2fa`) — checkpoints and 2FA are different Instagram protocols. **This talks to Instagram's undocumented private "challenge/resolve" API** (step_name/step_data shape, and the choice-value-to-contact-method mapping) — it follows patterns observed in community Instagram API clients but has not been verified end-to-end against a real flagged account. If it doesn't fully resolve a real checkpoint, check `[instagram-client]` logs for the raw `step_name`/`step_data` Instagram actually returned and adjust the choice-extraction logic in `fetchChallengeContext` accordingly.
